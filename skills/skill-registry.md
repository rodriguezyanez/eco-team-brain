...
| Auditoria de Certificacion | `auditoria.md` | Audita un entregable (Spring Boot / Lambda / API / Angular) antes de certificacion: predice los gates de Jenkins (Sonar, OWASP Dependency-Check, Trivy) + estandar KLAP y emite veredicto APTO / NO APTO con informe Markdown + dashboard HTML |
| Web Artifacts Builder | `web-artifacts-builder.md` | Construye dashboards/artefactos HTML autocontenidos (una pagina, sin CDNs, CSP-safe) con graficos SVG inline; usada por `/auditoria` y reutilizable en cualquier proyecto |
