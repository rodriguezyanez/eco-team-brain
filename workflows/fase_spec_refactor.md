...
- [ ] **Comportamiento actual documentado**
  - Contratos de API expuestos (endpoints REST, topics Kafka publicados/consumidos)
...
- [ ] **Plan de compatibilidad** — cómo se preservan los contratos externos durante la migración:
  - APIs REST: ¿mismo path, mismos campos, mismo status code?
  - Topics Kafka: ¿mismo schema, mismo topic name?
  - BD: ¿mismas tablas/columnas o migración Flyway necesaria?
...
En proyectos de refactorización, el plan de tasks debe incluir una fase de **characterization tests** (tests que capturan el comportamiento actual antes de tocar el código) cuando el proyecto existente no tiene tests suficientes. Estos tests son el contrato de seguridad durante la refactorización.
...
- [ ] **Tabla de tareas atómicas** con orden de dependencias, columna **Ámbito** y columna **Riesgo** — incluir characterization tests ANTES de las tareas de refactorización cuando corresponda.
...
- [ ] **Plan de tests por tarea** (unitarios e integración distinguidos explícitamente):
  - Characterization tests del comportamiento actual (si aplica)
  - Tests unitarios de la nueva implementación
  - Tests de integración por componente que interactúe con infraestructura externa
  - Tests de regresión que validen que los contratos externos se preservan
